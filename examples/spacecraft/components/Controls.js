...
export function Controls({ index, total, regions, activeId, controllerRef }) {
  ...
  const part = (id, name, accent) =>
    h(
      "button",
      {
        key: id,
        type: "button",
        className: `sc-part${id === activeId ? " sc-part-on" : ""}`,
  ...
  const icon = (cls, title, onClick) =>
    h("button", { type: "button", className: "sc-icon", title, ariaLabel: title, onClick }, h("i", { className: cls, ariaHidden: "true" }));
  ...
  return h(
    ...
    h(
      "div",
      { className: "sc-parts", role: "group", ariaLabel: "Jump to a module" },
      part("overview", "Overview"),
      regions.map((r) => part(r.id, r.name, r.accent)),
    ),
    h(
      "div",
      { className: "sc-toolbar", role: "group", ariaLabel: "View controls" },
      h("button", { type: "button", className: "sc-btn", title: "Fit everything", onClick: () => c()?.fitAll() },
        h("i", { className: "bi bi-arrows-fullscreen", ariaHidden: "true" }), h("span", null, "Overview")),
      h("button", { type: "button", className: "sc-btn", title: "Recenter the module (Esc)", onClick: () => c()?.reset() },
        h("i", { className: "bi bi-bullseye", ariaHidden: "true" }), h("span", null, "Recenter")),
      h("span", { className: "sc-group" },
        icon("bi bi-dash-lg", "Zoom out", () => c()?.zoomOut()),
        icon("bi bi-plus-lg", "Zoom in", () => c()?.zoomIn())),
      h("span", { className: "sc-group" },
        icon("bi bi-chevron-left", "Previous", () => c()?.prev()),
        h("span", { className: "sc-count" }, `${index + 1} / ${total}`),
