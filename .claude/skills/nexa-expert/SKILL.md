...
- **SSR**: `renderToString(App)` (from `/dist/nexa-server.js`) renders a component/vnode to an HTML string with no DOM — `useState`/`useMemo`/`useContext` run, `useEffect` does not, values are HTML-escaped, handlers/refs omitted (AI_SPEC §6). Keep browser-only work (fetch, `window`, `localStorage`) inside effects so components stay server-renderable. Hydration is not implemented yet.
