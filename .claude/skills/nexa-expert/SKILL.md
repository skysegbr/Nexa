...
- **Types & tooling**: TypeScript declarations ship in `dist/nexa.d.ts` / `dist/nexa-components.d.ts` (no build — just `.d.ts`). Repo commands run directly with Python (no Node/npm): `python server.py` (dev server + HMR), `python scripts/run_browser_tests.py` (headless test suite), `python scripts/validate_nexa.py`, `python scripts/minify.py`. After editing any `dist/*.js` or `*.css`, re-run `python scripts/minify.py` so the committed `*.min.*` stay in sync, and run the test suite.
- **SSR**: `renderToString(App)` (from `/dist/nexa-server.js`) renders a component/vnode to an HTML string with no DOM — `useState`/`useMemo`/`useContext` run, `useEffect` does not, values are HTML-escaped, handlers/refs omitted. `hydrate(App, container)` then adopts that server HTML in place (reusing nodes, attaching handlers) instead of recreating it (AI_SPEC §6). Keep browser-only work (fetch, `window`, `localStorage`) inside effects so components stay server-renderable.
