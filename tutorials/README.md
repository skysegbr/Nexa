...
| [ssr](./ssr/) | `ssr-tutorial.webm` (~38 s) | SSR (`examples/ssr` live in an iframe): `renderToString` producing the raw HTML string (shown on camera), one component on both sides, `hydrate` adopting the server DOM with the node-reuse proof, and the hydrated buttons clicked live |
